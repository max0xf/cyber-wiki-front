...
const rule = {
    meta: {
        ...
        messages: {
            invalidEventFormat: 'Event enum value must use template literal with domain pattern. ' +
                'Expected: `${SCREENSET_ID}/${DOMAIN_ID}/{{eventName}}` ' +
                'This ensures events are properly namespaced and auto-update when IDs change.',
            missingDomainInPattern: 'Event enum value is missing domain component. ' +
    ...
    create(context) {
        ...
        return {
            ...
            'TSEnumMember > Literal[value=/\//]'(node) {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const enumMember = node.parent;
                context.report({
                    node: enumMember,
                    messageId: 'invalidEventFormat',
...
module.exports = rule;
//# sourceMappingURL=domain-event-format.js.map